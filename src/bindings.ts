...
async updateSymbolMappings(mappings: SymbolMapping[]) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("update_symbol_mappings", { mappings }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type SymbolMapping = { phrase: string; symbol: string }
